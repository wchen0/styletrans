...
from WCT_TF.WCT_TF.ops import wct_np
from WCT_TF.WCT_TF.coral import coral_numpy
import scipy.misc
from WCT_TF.WCT_TF.utils import swap_filter_fit, center_crop_to
...
class WCT(object):
    ...
    def __init__(self,
        ...
        with graph.device(device):
            ...
            for relu_target, checkpoint_dir in zip(relu_targets, checkpoints):
                ...
                saver = tf.compat.v1.train.Saver(var_list=relu_vars)
    ...
    def predict(self,
        ...
        if swap5 is True and self.ss_stride != 1:
            ...
            should_refit, H, W = swap_filter_fit(old_H, old_W,
                                                 self.ss_patch_size,
                                                 self.ss_stride)
            if should_refit:
        ...
        s = time.time()
        stylized = self.sess.run(self.decoded_output,
                                 feed_dict={
