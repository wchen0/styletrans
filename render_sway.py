...
import torch
from photostyle.process_stylization import stylization
from photostyle.photo_wct import PhotoWCT
...
flags.DEFINE_string(
    "input_file", default="", help="Input batch filename")
flags.DEFINE_string(
    "output_dir", default="", help="Directory to save MPI planes and renderings")
flags.DEFINE_string(
    "model_dir", default="./mpi_extrapolation/models/", help="Directory containing pretrained model")
flags.DEFINE_string(
    "style_image_path", default="./images/style.png", help="Input style image")
flags.DEFINE_string(
    "styletrans_model", default="./photostyle/PhotoWCTModels/photo_wct.pth", help="File of style transfer model")
flags.DEFINE_string(
    "fast", default="False", help="Fast style transfer")
flags.DEFINE_string(
    "cuda", default="1", help="cuda")
...
max_disp = 128.0  # maximum pixel disparity of closest plane (1.0 m) in sway
...
def main(argv):
  ...
  if FLAGS.styletrans_model is None:
    raise ValueError("`styletrans_model` must be defined")

  # Set up model for style transfer
  p_wct = PhotoWCT()
  p_wct.load_state_dict(torch.load(FLAGS.styletrans_model))

  if FLAGS.fast:
    from photostyle.photo_gif import GIFSmoothing
    p_pro = GIFSmoothing(r=35, eps=0.001)
  else:
    from photostyle.photo_smooth import Propagator
    p_pro = Propagator()
  if FLAGS.cuda:
    p_wct.cuda(0)

  # Set up model for multiplane images
  model = MPI()
  ...
  # in_tgt_image = tf.constant(inputs["tgt_image"])   # Unneeded for sway
  ...
  for i in range(layers.shape[3]):
    ...
    # Set maximum 1 and minimum 0
    # Save layer before stylization
    plt.imsave(i_filename, np.clip(layers[0, :, :, i, :], 0.0, 1.0))
    
    stylized_i_filename = FLAGS.output_dir + "stylized_layer_{:04d}.png".format(i)
    # Stylize layer and save it
    stylization(
              stylization_module=p_wct,
              smoothing_module=p_pro,
              content_image_path=i_filename,
              style_image_path=FLAGS.style_image_path,
              content_seg_path=[],
              style_seg_path=[],
              output_image_path=stylized_i_filename,
              cuda=1,
              save_intermediate=False,
              no_post=False
    )
    print(stylized_i_filename, "saved")

    # reload image
    stylized_layer = np.array(Image.open(stylized_i_filename).convert('RGB'))
    stylized_layer = stylized_layer.astype(np.float32) / 255
    
    # rebuild MPI
    outs["rgba_layers_refine"][0, :, :, i, 0] = stylized_layer[:, :, 0]
    outs["rgba_layers_refine"][0, :, :, i, 1] = stylized_layer[:, :, 1]
    outs["rgba_layers_refine"][0, :, :, i, 2] = stylized_layer[:, :, 2]
  ...
  for i in range(len(output_render_list)):
    plt.imsave(FLAGS.output_dir + "tmp_{:03d}.png".format(i),
               np.clip(output_render_list[i][0, :, :, :], 0.0, 1.0))
  ...
  # Generate gif
  subprocess.call([
      "ffmpeg", "-i", FLAGS.output_dir + "sway.mp4",  FLAGS.output_dir + "sway.gif"
  ])
