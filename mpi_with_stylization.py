...
from WCT_TF.WCT_TF.wct import WCT
from WCT_TF.WCT_TF.model import WCTModel
from WCT_TF.WCT_TF.utils import preserve_colors_np
from WCT_TF.WCT_TF.utils import resize_to, get_img
import scipy
import time


@tf.function
def stylization(wct_model, content_img, style_img, alpha=0.5):
    if isinstance(style_img, str):
        style_img = get_img(style_img)
    style_img = resize_to(style_img, 512)

    pred = content_img

    for key in pred:
        for img in pred[key]:
            stylized_rgb = wct_model.predict(content=pred[key],
                                             style=style_img,
                                             alpha=alpha,
                                             swap5=False,
                                             ss_alpha=0.6,
...
class MPI(object):
    ...
    """
        ...
        with tf.name_scope("format_network_input"):
            ...
            net_input = self.format_network_input(ref_image,
                                                  src_images[:, :, :, 3:],
        ...
        with tf.name_scope("layer_prediction"):
            ...
            if concat_trans:
                refine_input = tf.concat([
                    tf.stop_gradient(refine_input_colors),
                    tf.stop_gradient(stuff_behind),
                    tf.stop_gradient(refine_input_alpha),
                    tf.stop_gradient(transmittance)
                ],
                                         axis=4)

            normalized_disp_inds = tf.reshape(
                tf.linspace(0.0, 1.0, num_mpi_planes),
                [1, 1, 1, num_mpi_planes, 1])
            sh = tf.shape(refine_input)
            normalized_disp_inds_stack = tf.tile(normalized_disp_inds,
                                                 [1, sh[1], sh[2], 1, 1])
            refine_input = tf.concat(
                [refine_input, normalized_disp_inds_stack], axis=4)
            ...
            # Rescale alphas to (0, 1)
            alpha_layers = (alpha_layers + 1.) / 2.
            rgba_layers_refine = tf.concat([color_layers, alpha_layers],
                                           axis=4)
        ...
        pred["refine_input_mpi"] = tf.concat(
            [refine_input_colors, refine_input_alpha], axis=-1)
    ...
    Args:
      ...
      vgg_model_file: path to vgg weights
    Returns:
      A train_op to be used for training.
    """
        print("setting up WCT model")
        # wct_model = WCT(checkpoints=wct_model_dirs,
        #                 relu_targets=wct_relu_targets,
        #                 vgg_path=vgg_path_for_wct,
        #                 device=device_wct,
        ...
        print("starting to build graph for MPI")
        with tf.name_scope("input_size_randomization"):
            dim_choices = tf.constant([[1, 16], [2, 32], [4, 32], [4, 64],
                                       [4, 128], [8, 32], [8, 64], [8, 128]],
                                      dtype=tf.int32)
        ...
        with tf.name_scope("input_data"):
            ...
            # size x height x width x channel
            _, img_height, img_width, _ = tf.convert_to_tensor(
                raw_src_images).get_shape().as_list()

            img_height = img_height // height_div
            img_width = img_width // width_div

            raw_tgt_image = tf.image.convert_image_dtype(raw_tgt_image,
                                                         dtype=tf.float32)
            raw_ref_image = tf.image.convert_image_dtype(raw_ref_image,
                                                         dtype=tf.float32)
            raw_src_images = tf.image.convert_image_dtype(raw_src_images,
                                                          dtype=tf.float32)
            ...
            intrinsics = tf.concat([
                ...
                intrinsics[:, 1:2, :] / tf.to_float(height_div),
                intrinsics[:, 2:3, :]
            ],
                                   axis=1)
        ...
        with tf.name_scope("inference"):
            ...
            pred = self.infer_mpi(raw_src_images, raw_ref_image, ref_pose,
                                  src_poses, intrinsics, num_mpi_planes,
                                  mpi_planes)

            # # stylizing every MPI layer before synthesis
            # print("stylizing MPIs")
            # stylization(wct_model=wct_model,
            #             content_img=pred,
        ...
        with tf.name_scope("loss"):
            ...
            loss_mask = tf.where(
                tf.equal(
                    tf.reduce_min(tf.abs(tf.reduce_sum(output_layers,
                                                       axis=-1)),
                                  axis=3,
                                  keep_dims=True), 0.0),
            ...
            downsample = lambda tensor, ds: tf.nn.avg_pool(
                tensor, [1, ds, ds, 1], [1, ds, ds, 1], "SAME")
            ...
            def vgg_loss(raw_tgt_image, output_image, loss_mask):
                ...
                vgg_fake = build_vgg19(rescaled_output_image,
                                       vgg_model_file,
                                       reuse=True)
            ...
            # loss between `raw_tgt_image` and `output_image`
            vgg_loss_initial, _, _ = vgg_loss(raw_tgt_image, output_image,
                                              loss_mask)
            tf.summary.scalar("vgg_loss_initial", vgg_loss_initial)
            total_loss += vgg_loss_initial

            # loss between `raw_tgt_image` and `output_image_refine`
            vgg_loss_refine, _, _ = vgg_loss(raw_tgt_image,
                                             output_image_refine, loss_mask)
        ...
        for i in range(num_summ):
            ...
            tf.summary.image("rgba_layer_%d" % i,
            ...
            tf.summary.image("psv_source_%d" % i,
                             self.deprocess_image(source_plane))
        '''
        ...
        for i in range(num_summ):
            ...
            tf.summary.image("rgb_layer_refine_%d" % i,
                             self.deprocess_image(rgb))
            tf.summary.image("alpha_layer_refine_%d" % i, alpha)
            tf.summary.image("rgba_layer_refine_%d" % i,
                             self.deprocess_image(rgb * alpha))
            tf.summary.image("cumulative_render_%d" % i,
                             self.deprocess_image(render))
    ...
    """
        ...
        #     [tf.reduce_prod(tf.shape(v)) for v in tf.trainable_variables()])  # unused
        global_step = tf.Variable(0, name='global_step',
                                  trainable=False)  # this line was added
        incr_global_step = tf.assign(global_step, global_step + 1)
        saver = tf.train.Saver([var for var in tf.trainable_variables()] +
                               [global_step],
                               max_to_keep=None)
        sv = tf.train.Supervisor(logdir=summary_dir,
        ...
        with sv.managed_session() as sess:
            ...
            for step in range(1, max_steps):
                ...
                if step % summary_freq == 0:
                    ...
                    print("[Step %.8d] time: %4.4f/it" %
                          (gs, time.time() - start_time))
    ...
    """
        depths = 1.0 / tf.linspace(1.0 / end_depth, 1.0 / start_depth,
                                   num_depths)
        return depths
    ...
    """
        ...
        trans_ref_reproj = pj.projective_forward_homography(
            trans_ref, intrinsics, rel_pose, depths)
        ...
        trans_target = self.compute_transmittance(
            tf.stop_gradient(output_alpha))
        trans_target = tf.expand_dims(trans_target, -1)

        # One-sided occlusion map (positive diff in transmittance of target vs. ref)
        occ_map = tf.reduce_max(tf.nn.relu(trans_target - trans_ref_reproj),
                                axis=3)
